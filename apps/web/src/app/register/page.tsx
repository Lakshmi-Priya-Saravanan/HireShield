      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <Card className="bg-slate-900/50 border-slate-800 backdrop-blur-md shadow-2xl">
          <CardHeader className="text-center">
            ...
            <CardDescription className="text-slate-400">Register to track and audit employment leads.</CardDescription>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                <label className="text-sm font-medium text-slate-300">Profile Purpose</label>
                <select
                  value={role}
                  onChange={(e) => setRole(e.target.value)}
                  className="w-full px-3 py-2 bg-slate-950 border border-slate-800 text-slate-200 rounded-md focus:outline-none focus:ring-1 focus:ring-blue-500"
                >
                  <option value="SEEKER">Job Seeker (Audit Offers & Emails)</option>
                  <option value="RECRUITER">Recruiter (Verify Authenticity)</option>
                </select>
              </div>

              <Button type="submit" className="w-full bg-blue-600 hover:bg-blue-500 mt-2" disabled={loading}>
                {loading ? (
                  <span className="flex items-center gap-2 justify-center"><Activity className="w-4 h-4 animate-spin" /> Creating profile...</span>
                ) : "Create Account"}
              </Button>
            </form>
          </CardContent>
